...
async function generateShareSummary(
  ...
  const sys =
    ...
    '(2) 判斷文章「視覺敏感度」(sensitive)：判準不是「話題是否觸及敏感」，而是「要為這篇文章畫宣傳圖時，圖本身是否必須描繪露骨/裸露/血腥/性暗示等內容才能呈現主題」。' +
    '範例：' +
    '  - "NSFW 模型平台比較研究"、"成人產業稅務分析"、"性教育課程設計" → 都是學術 / 比較 / 知識性討論，圖可以畫女孩拿筆記本/平板分析比較，**sensitive=false**。' +
    '  - "我跟前任最後一夜的故事"、"如何挑選成人玩具"、"親身體驗 BDSM 心得" → 圖會自然帶到露骨情境，**sensitive=true**。' +
    '  - 純政治評論、暴力新聞分析、毒品政策辯論 → 圖可抽象表達，**sensitive=false**。' +
    '只有當「合理的宣傳圖會自然滑向 NSFW」才標 true，其他全部 false。' +
